...
let previous = '', version=0, sequence=0;
window.namiOverlay.onRender(data => {
...
for (const type of ['click','input','change','keydown','focusin','submit']) document.addEventListener(type, event => {
  ...
  window.namiOverlay.input({ version, sequence, type: type === 'focusin' ? 'focus' : type, target: Number(target.dataset.overlayNode), value: target.value, checked: target.checked, key: event.key, shiftKey: event.shiftKey });
});
