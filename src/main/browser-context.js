...
class SessionContextStore {
  ...
  update({ id, windowId, identity, title, kind, content, truncated = false }) {
    ...
    const source = { id, windowId, identity: clean(identity, 200), title: clean(title, 200), kind,
      content: clean(text, 64000), truncated: !!truncated || text.length > 64000, capturedAt: Date.now() };
    this.sources.set(id, source); return this.describe(source);
  }
  describe(s) { return { id: s.id, title: s.title, kind: s.kind, capturedAt: s.capturedAt, truncated: s.truncated, identity: s.identity }; }
  ...
  read(recipient, id) {
    ...
    return { ...this.describe(s), content: s.content, incompleteHistory: s.kind === 'terminal' || s.truncated };
  }
