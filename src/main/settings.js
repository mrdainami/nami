...
// The four themes the app ships; anything else falls back to paper so a stale
// or hand-edited settings.json can never paint the window an unknown color.
...
function normalizeTheme(name) { return THEMES.includes(name) ? name : 'paper'; }
function themeBackground(name) { return THEME_BG[normalizeTheme(name)]; }

module.exports = { readSettings, writeSettings, fsIo, normalizeTheme, themeBackground, THEMES };
