...
let active = false, mode = 'component', pointer = null, frame = 0, sequence = 0, suppressClick = false;
...
function select(el, range, rect) {
  ...
  ipcRenderer.send('browser:selection', { ...geometry(item), documentId, kind, viewport: viewport(), text,
    locator: kind === 'region' ? '' : locator(el), label: kind === 'region' ? 'Visual region (no image attached)' : kind === 'text' ? 'Selected text' : el?.tagName?.toLowerCase() || 'Page component', title: document.title });
  active = false; pointer = null; schedule();
}
ipcRenderer.on('browser:annotate-mode', (_e, value) => {
  active = typeof value === 'object' ? !!value.active : !!value;
  mode = ['component', 'text', 'region'].includes(value?.mode) ? value.mode : 'component'; pointer = null; suppressClick = false; schedule();
});
...
window.addEventListener('keydown', (event) => {
  if (event.key === 'Escape' && active) { event.preventDefault(); event.stopImmediatePropagation(); active = false; pointer = null; suppressClick = false; ipcRenderer.send('browser:annotation-end'); }
}, true);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  new MutationObserver((changes) => {
    ...
    if (changes.length) for (const item of tracked.values()) if (item.kind === 'region') item.stale = true;
    schedule();
